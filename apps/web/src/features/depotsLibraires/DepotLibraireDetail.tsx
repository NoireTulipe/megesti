...
const TABS: { id: TabId; label: string; icon: string }[] = [
  ...
  { id: 'stock',    label: 'Stock déposé', icon: 'M20 7l-8-4-8 4m16 0-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
...
function TabInfos({ depot, depotId }: { depot: ReturnType<typeof useDepotLibraireDetail>['data']; depotId: string }) {
  ...
  if (!edit) return (
    <div className={sty['profil-grid']}>
      ...
      <div className={sty['profil-field']}><label>Adresse</label><span>{depot.adresse || '—'}</span></div>
      <div className={sty['profil-field']}><label>Commission fixe</label><span>{depot.commissionFixe ? fmtEuro(Number(depot.commissionFixe)) : '—'}</span></div>
      <div className={sty['profil-field']}><label>Commission %</label><span>{depot.commissionPourcent ? `${Number(depot.commissionPourcent)} %` : '—'}</span></div>
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        {[
          ...
          { label:'Adresse', val:addr, set:setAddr, placeholder:'Rue, ville…' },
          { label:'Commission fixe (€)', val:cFix, set:setCFix, placeholder:'0.00', type:'number' },
...
function TabContacts({ depot, depotId }: { depot: ReturnType<typeof useDepotLibraireDetail>['data']; depotId: string }) {
  ...
  const empty = { id: crypto.randomUUID(), nom:'', prenom:'', email:'', telephone:'' }
  const [f, setF] = useState(empty)

  const contacts = depot?.contacts ?? []

  function openNew()          { setF({ ...empty, id: crypto.randomUUID() }); setEditing('new') }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      {contacts.length === 0 && !editing && (
        <p style={{ color:'var(--text-soft)', fontStyle:'italic', fontSize:'0.85rem' }}>Aucun contact enregistré.</p>
      )}
      {contacts.map(c => (
      ...
      {editing && (
        <div style={{ padding:16, background:'#fff', border:'1.5px solid var(--terra-light)', borderRadius:14, display:'flex', flexDirection:'column', gap:12 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            {field('Prénom',     f.prenom??'',v => setF(s=>({...s,prenom:v})))}
            {field('Email',      f.email??'', v => setF(s=>({...s,email:v})), 'contact@librairie.fr')}
            {field('Téléphone',  f.telephone??'', v => setF(s=>({...s,telephone:v})), '01 23 45 67 89')}
          </div>
          <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
...
function TabStock({ depot, depotId }: { depot: ReturnType<typeof useDepotLibraireDetail>['data']; depotId: string }) {
  ...
  async function sendArticle() {
    ...
    await envoyer.mutateAsync({ id: crypto.randomUUID(), articleId, quantiteEnvoyee: Number(qte), notes: notes || undefined })
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      {stock.length === 0 && !showForm && (
        <p style={{ color:'var(--text-soft)', fontStyle:'italic', fontSize:'0.85rem' }}>Aucun article en dépôt pour l'instant.</p>
      )}
          ...
          <div key={a.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'12px 14px', background:'var(--cream)', border:'1px solid var(--cream-dark)', borderRadius:14 }}>
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:'0.72rem', color:'var(--text-soft)', marginTop:3, display:'flex', gap:12 }}>
                <span>Envoyé : <strong>{a.quantiteEnvoyee}</strong></span>
            ...
            <button type="button" onClick={() => retirer.mutate(a.id)} title="Retirer du dépôt (retour stock)"
      ...
      {showForm && (
        <div style={{ padding:16, background:'#fff', border:'1.5px solid var(--terra-light)', borderRadius:14, display:'flex', flexDirection:'column', gap:12 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
              <label style={{ fontSize:'0.72rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.07em', color:'var(--text-soft)' }}>Quantité *</label>
          ...
          <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
            ...
            <button type="button" onClick={sendArticle} disabled={!articleId || !qte || envoyer.isPending} style={{ height:34, padding:'0 18px', background:'var(--ink-mid)', color:'#fff', border:'none', borderRadius:'var(--r-pill)', fontSize:'0.82rem', fontWeight:600, cursor:'pointer' }}>
              {envoyer.isPending ? 'Envoi…' : 'Envoyer en dépôt'}
            </button>
          </div>
        </div>
      )}
...
function TabVentes({ depot, depotId }: { depot: ReturnType<typeof useDepotLibraireDetail>['data']; depotId: string }) {
  ...
  async function confirm() {
    ...
    setSuccess(`Vente enregistrée${comm}`)
    setSelected(''); setQte('')
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <div style={{ padding:20, background:'var(--cream)', border:'1px solid var(--cream-dark)', borderRadius:16 }}>
        ...
        {stock.length === 0 ? (
          <p style={{ color:'var(--text-soft)', fontStyle:'italic', fontSize:'0.85rem' }}>Aucun article disponible à la vente dans ce dépôt.</p>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                ...
                <select value={selected} onChange={e => setSelected(e.target.value)} style={{ padding:'9px 12px', border:'1.5px solid var(--cream-dark)', borderRadius:10, fontSize:'0.875rem', outline:'none', background:'#fff' }}>
                  ...
                  {stock.map(a => (
                    <option key={a.id} value={a.id}>
                      {a.article.nom} · {a.quantiteEnvoyee - a.quantiteVendue} dispo
                    </option>
                  ))}
                </select>
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                <label style={{ fontSize:'0.72rem', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.07em', color:'var(--text-soft)' }}>Qté vendue</label>
      ...
      <div>
        <p style={{ fontWeight:700, fontSize:'0.82rem', color:'var(--text-soft)', marginBottom:10, textTransform:'uppercase', letterSpacing:'0.08em' }}>Récapitulatif dépôt</p>
            ...
            <div className={sty['detail-hero-info']}>
              ...
              <div className={sty['detail-hero-pills']}>
                ...
                <span className={`${sty['detail-hero-pill']} ${sty['detail-hero-pill-livres']}`}>
                  ...
                  {nbRestant} ex. en dépôt
                </span>
                {(depot.commissionPourcent || depot.commissionFixe) && (
                  <span className={sty['detail-hero-pill']}>
                    Commission {depot.commissionPourcent ? `${Number(depot.commissionPourcent)} %` : `${Number(depot.commissionFixe).toFixed(2)} €`}
                  </span>
                )}
              </div>
            </div>
